                    ...
                    <div class="packages-content bg-light">
                        ...
                        <div class="row bg-primary rounded-bottom mx-0">
                            <div class="col-6 text-start px-0">
                                ...
                                <a href="{{ $package->read_more_link }}" class="btn-hover btn text-white py-2 px-4" data-ajax-link>
                                    {{ $package->read_more_text ?: 'Read More' }}
                                </a>
                                @else
                                <button class="btn-hover btn text-white py-2 px-4" onclick="alert('Package details coming soon!')">
                                    {{ $package->read_more_text ?: 'Read More' }}
                                </button>
                                @endif
                            </div>
                            <div class="col-6 text-end px-0">
                                ...
                                <a href="{{ $package->book_now_link }}" class="btn-hover btn text-white py-2 px-4" data-ajax-link>
                                    {{ $package->book_now_text ?: 'Book Now' }}
                                </a>
                                @else
                                <a href="{{ route('booking') }}" class="btn-hover btn text-white py-2 px-4" data-ajax-link>
                                    {{ $package->book_now_text ?: 'Book Now' }}
                                </a>
                                @endif
                            </div>
...
@push('scripts')
<script>
    // Handle AJAX links within the content
    $(document).on('click', '[data-ajax-link]', function(e) {
        const href = $(this).attr('href');
        if (href && href !== '#' && !href.startsWith('http')) {
            e.preventDefault();
            // Trigger the main navigation handler
            $(this).trigger('click');
        }
    });
</script>
